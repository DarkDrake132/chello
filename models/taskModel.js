...
async function getTaskById(taskId) {
    ...
    let res = taskData[0];
    if (new Date(res.dueDate) < new Date()) res.isExpired = true;
    else res.isExpired = false;
...
async function addTask(data) {
    let query = `
        ...
        VALUES ('${data.taskId}','${data.taskName}','${data.description}','${
        data.haveDeadline
    }','${dateReformat(data.dueDate)}','${data.isExpired}','${data.isHidden}')
    `;
...
async function updateTaskById(data) {
    ...
    let updateQuery = `
        UPDATE task 
        SET name='${data.taskName}', description='${
        data.description
    }', have_deadline='${data.haveDeadline}', duedate='${dateReformat(
        data.dueDate
    )}', is_expired='${data.isExpired}', is_hidden='${data.isHidden}'
